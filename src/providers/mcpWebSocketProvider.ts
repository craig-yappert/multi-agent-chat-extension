...
// Dynamic import for WebSocket to handle missing dependency
let WebSocket: any;
try {
	WebSocket = require('ws');
} catch (e) {
	console.warn('WebSocket module not available, MCP WebSocket features disabled');
}
...
export class MCPWebSocketProvider {
	...
	private connect(): void {
		if (!WebSocket) {
			console.warn('[MCP-WS] WebSocket module not available, skipping connection');
			return;
		}
		try {
	...
	private setupHeartbeat(): void {
		...
		this.heartbeatInterval = setInterval(() => {
			if (this.isConnected && WebSocket && this.ws?.readyState === WebSocket.OPEN) {
	 ...
	 */
	isReady(): boolean {
		return this.isConnected && WebSocket && this.ws?.readyState === WebSocket.OPEN;
	}
