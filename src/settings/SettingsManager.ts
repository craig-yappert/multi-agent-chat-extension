...
export interface GlobalSettings {
    // API keys moved to SecretStorage (see ApiKeyManager)
    globalOptions?: {
        yoloMode?: boolean;
...
export class SettingsManager {
    ...
    private loadVSCodeSettings(): Settings {
        ...
        return {
            // API keys now loaded from SecretStorage via ApiKeyManager
    ...
    private mergeSettings(...settingsArray: Settings[]): Settings {
        ...
        for (const settings of settingsArray) {
            ...
            // API keys removed - now in SecretStorage
    ...
    public async ensureMachatStructure(): Promise<void> {
        ...
        if (!await this.fileExists(gitignorePath)) {
            const gitignoreContent = `# Multi Agent Chat - Project Gitignore
# Generated by Multi Agent Chat extension

# Conversations (may contain sensitive data or be user-specific)
conversations/

# Agent memory and context (may contain project secrets)
...
# - config.json (project settings, NO API keys - they're in SecretStorage)
# - models.json (project-specific model configurations)
# - agents.json (project-specific agent configurations)
# - context/project-context.md (project documentation)
# - context/knowledge-base/ (shared knowledge)
`;
     ...
     * @deprecated Use ApiKeyManager.getInstance(context).getApiKey(provider) directly
     */
